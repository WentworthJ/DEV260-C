...
public class FileNode
{
    ...
    public FileNode(string name, FileType type, long size)
    {
        ...
        Type = type;
        Size = size;
        Extension = type == FileType.File && name.Contains(".") 
...
namespace FileSystemNavigator
{
    ...
    public class FileSystemBST
    {
        ...
        public bool CreateFile(string fileName, long size = 1024)
        {
            ...
            var fileNode = new FileNode(fileName, FileType.File, size);
        ...
        public bool CreateDirectory(string directoryName)
        {
            ...
            var dirNode = new FileNode(directoryName, FileType.Directory, 0);
        ...
        public List<FileNode> FindFilesByExtension(string extension)
        {
            ...
            TraverseAndCollect(root, list, f => f.Type == FileType.File && 
        ...
        public List<FileNode> FindFilesBySize(long minSize, long maxSize)
        {
            ...
            var list = new List<FileNode>();
            TraverseAndCollect(root, list, f => f.Type == FileType.File && f.Size >= minSize && f.Size <= maxSize);
            return list;
        ...
        public List<FileNode> FindLargestFiles(int count)
        {
            ...
            return list.OrderByDescending(f => f.Size).Take(count).ToList();
        ...
        public long CalculateTotalSize()
        {
            ...
            long TotalSize(TreeNode? node)
            {
                if (node == null) return 0;
                return node.FileData.Size + TotalSize(node.Left) + TotalSize(node.Right);
            }
        ...
        public bool DeleteItem(string fileName)
        {
            ...
            bool deleted;
            root = DeleteNodeRecursive(root, fileName, out deleted);
            return deleted;
        ...
        private TreeNode? InsertNode(TreeNode? node, FileNode fileData)
        {
            ...
            if (comparison < 0)
                ...
                return node; // duplicate, do not insert
        ...
        //-------------------------------
        private TreeNode? DeleteNodeRecursive(TreeNode? node, string fileName, out bool deleted)
{
        if (node == null)
        {
            deleted = false;
        ...
        if (comparison < 0)
        {
            node.Left = DeleteNodeRecursive(node.Left, fileName, out deleted);
        }
        else if (comparison > 0)
        {
            node.Right = DeleteNodeRecursive(node.Right, fileName, out deleted);
        }
        else
        {
            // Node found
            ...
            node.Right = DeleteNodeRecursive(node.Right, successor.FileData.Name, out _);
        }
