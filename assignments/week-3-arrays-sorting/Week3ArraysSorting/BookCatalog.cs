...
namespace Week3ArraysSorting
{
    ...
    public class BookCatalog
    {
        ...
        private void SortBooksRecursively()
        {
            ...
            Console.WriteLine("Sorting books using recursive MergeSort...");
        ...
        private void Merge(string[] norm, string[] orig, int left, int mid, int right)
        {
            ...
            while (i1 < n1 && i2 < n2)
            {
                if (string.Compare(leftNorm[i1], rightNorm[i2], StringComparison.Ordinal) <= 0)
                {
                    norm[k] = leftNorm[i1];
                    orig[k] = leftOrig[i1];
                    i1++;
        ...
        private void BuildMultiDimensionalIndex()
        {
            ...
            ////
            Console.WriteLine("Building multi-dimensional index...");
            ...
            {
                string title = normalizedTitles[idx];
                if (title.Length == 0) continue;

                int first = GetLetterIndex(title[0]);
                int second = title.Length > 1 ? GetLetterIndex(title[1]) : 0;

                if (startIndex[first, second] == -1)
                    startIndex[first, second] = idx;

                endIndex[first, second] = idx + 1;
            }
        ...
        private void PerformLookup(string query)
        {
            ...
            int second = normalizedQuery.Length > 1 ? GetLetterIndex(normalizedQuery[1]) : 0;
            ...
            if (start == -1 || end == -1)
            {
                ...
                ShowSuggestions(normalizedQuery, 5);
                return;
            }

            int foundIndex = BinarySearchInRange(normalizedQuery, start, end - 1);
            ...
            {
                Console.WriteLine("No exact match found. Suggestions:");
                ShowLocalSuggestions(start, end, 3);
            }
            ////
        }
        
        
        // adding methods for previous method
        ////
        ...
        private void ShowLocalSuggestions(int start, int end, int max)
        {
            int count = 0;
        ...
        private void ShowSuggestions(string query, int max)
        {
            int count = 0;
            foreach (var title in originalTitles)
            {
                if (title.ToUpperInvariant().Contains(query))
                {
