...
namespace Week4DoublyLinkedLists.Applications
{
    ...
    public class MusicPlaylist
    {
        ...
        public bool RemoveSong(Song song)
        {
            ...
            bool wasCurrent = (node == currentSong);
            bool removed = playlist.Remove(song);
            
            if (wasCurrent)
            {
                if (node.Next != null) currentSong = node.Next;
                else if (node.Previous != null) currentSong = node.Previous;
        ...
        public bool RemoveSongAt(int position)
        {
            ...
            if (position < 0 || position >= TotalSongs) return false;
            ...
            bool wasCurrent = (current == currentSong);
            var removedSong = playlist.RemoveAt(position);
            bool removed = removedSong != null;

            if (wasCurrent)
            {
                if (current.Next != null) currentSong = current.Next;
                else if (current.Previous != null) currentSong = current.Previous;
                else currentSong = null;
            }
            return removed;
        ...
        public void DisplayPlaylist()
        {
            ...
            Console.WriteLine($"\n🎵 Playlist: {Name} ({TotalSongs} songs)");
            ...
            while (node != null)
            {
                string prefix = (node == currentSong) ? "►" : " ";
                Console.WriteLine($"{prefix} {index,2}. {node.Data}");
                node = node.Next;
                index++;
            }
