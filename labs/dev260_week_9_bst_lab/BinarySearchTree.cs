...
namespace Lab9_BST
{
    ...
    public class EmployeeManagementSystem
    {
        ...
        public Employee? FindMinimum()
        {
            ...
            //hrow new NotImplementedException("FindMinimum method needs implementation");
        }
        ...
        private TreeNode? InsertRecursive(TreeNode? node, Employee employee)
        {
            ...
            if (employee. EmployeeId < node. Employee.EmployeeId) { // Go left for smaller IDs
            node.Left = InsertRecursive (node. Left, employee);
            
            } else if (employee. EmployeeId> node. Employee. EmployeeId) { // Go right for larger IDs
        ...
        private Employee? SearchRecursive(TreeNode? node, int employeeId)
        {
            ...
            if (employeeId == node.Employee.EmployeeId) {
                ...
                return SearchRecursive (node.Left,employeeId); // Go left } else {
            }
        ...
        private void InOrderRecursive(TreeNode? node)
        {
            ...
            Console.WriteLine(node.Employee); // Process current node 
