...
namespace Assignment10
{
    ...
    public class FlightNetwork
    {
        ...
        public void AddAirport(Airport airport)
        {
            ...
            if (airport == null)
            {
                Console.WriteLine("Error: Cannot add null airport.");
                return;
            }

            if (string.IsNullOrWhiteSpace(airport.Code))
            {
                Console.WriteLine("Error: Airport code cannot be empty.");
                return;
            }
        ...
        public void AddFlight(Flight flight)
        {
            ...
            {
                string city = airportCities.ContainsKey(origin) ? airportCities[origin] : "Unknown";
                AddAirport(new Airport(origin, $"{origin} Airport", city, "USA"));
            }

            // Ensure destination airport exists
            if (!airports.ContainsKey(destination))
            {
                string city = airportCities.ContainsKey(destination) ? airportCities[destination] : "Unknown";
                AddAirport(new Airport(destination, $"{destination} Airport", city, "USA"));
            }
        ...
        public void LoadFlightsFromCSV(string filename)
        {
            ...
            for (int i = 1; i < lines.Length; i++)
            {
                ...
                try
                {
                    ...
                    Flight f = new Flight(origin, dest, airline, duration, cost);
                    AddFlight(f);
                    count++;
                }
                catch
            ...
            Console.WriteLine($"Successfully loaded {count} flights.");
        ...
        public void DisplayAllAirports()
        {
            ...
            foreach (var airport in airports.Values.OrderBy(a => a.Code))
            {
                ...
                Console.WriteLine($"{airport.Code,-5} {airport.City,-20} Connections: {connections}");
            }
        ...
        public Airport? GetAirport(string code)
        {
            ...
            return airports.ContainsKey(c) ? airports[c] : null;
        ...
        public List<string>? FindRoute(string origin, string destination)
        {
            ...
            while (queue.Count > 0)
            {
                ...
                {
                    string next = flight.Destination;
                    ...
                    if (!visited.Contains(next))
                    {
                        ...
                        if (next == destination)
                            return ReconstructPath(parents, origin, destination);
        ...
        public List<string>? FindCheapestRoute(string origin, string destination)
        {
            ...
            Dictionary<string, decimal> dist = airports.Keys.ToDictionary(k => k, k => decimal.MaxValue);
            ...
            while (pq.Count > 0)
            {
                pq.TryDequeue(out string current, out decimal currentCost);
                ...
                {
                    string next = flight.Destination;
                    decimal newCost = dist[current] + flight.Cost;
        ...
        public List<List<string>> FindRoutesByCriteria(string origin, string destination, int maxStops, decimal maxCost)
        {
            ...
            List<string> currentPath = new List<string> { origin };
            HashSet<string> visited = new HashSet<string> { origin };

            DFSWithConstraints(origin, destination, maxStops, maxCost,
                            0m, currentPath, visited, validRoutes);
            ...
            decimal currentCost, List<string> currentPath, HashSet<string> visited, List<List<string>> validRoutes)
        {
            ...
            if (current == destination)
            {
                validRoutes.Add(new List<string>(currentPath));  // copy the path
                return;
            }

            // Prune: too many stops (stops = path length - 1)
            ...
            foreach (var flight in routes[current])
            {
                ...
                // Explore
                DFSWithConstraints(next, destination, maxStops, maxCost, newCost, currentPath, visited, validRoutes);
        ...
        public string CalculateNetworkStatistics()
        {
            ...
            double avgConnections = (double)totalFlights / airports.Count;

            int maxConnections = routes.Max(kvp => kvp.Value.Count);
            int minConnections = routes.Min(kvp => kvp.Value.Count);

            var mostConnected = routes.Where(kvp => kvp.Value.Count == maxConnections).Select(kvp => kvp.Key);
            var leastConnected = routes.Where(kvp => kvp.Value.Count == minConnections).Select(kvp => kvp.Key);

            var allFlights = routes.Values.SelectMany(f => f).ToList();

            double avgCost = allFlights.Average(f => (double)f.Cost);
            ...
            return
                ...
                $"Most Connected ({maxConnections}): {string.Join(", ", mostConnected)}\n" +
                $"Least Connected ({minConnections}): {string.Join(", ", leastConnected)}\n" +
                $"Average Flight Cost: ${avgCost:F2}\n" +
                $"Average Duration: {avgDuration:F2} minutes";
